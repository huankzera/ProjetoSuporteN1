...
Uma ferramenta web interativa (**Single Page Application**) desenvolvida para auxiliar **Analistas de Suporte Nível 1** em Provedores de Internet (ISP). 

O objetivo é padronizar o atendimento, agilizar diagnósticos técnicos e fornecer scripts prontos para situações comuns, garantindo qualidade e rapidez no primeiro contato.

---

## 🚀 Funcionalidades

O sistema funciona como um "canivete suíço" para o atendimento:
...
- **📋 Scripts Inteligentes:** Respostas prontas para copiar e colar (com proteção *Anti-tabnabbing*), cobrindo cenários como CGNAT, IPTV, Lentidão em Redes Sociais e Bloqueios de Sites.
...
Mesmo sendo uma ferramenta de front-end, foram aplicadas práticas de segurança robustas:

1. **Content Security Policy (CSP):** Implementação de meta tag estrita para prevenir injeção de scripts (XSS).
2. **Proteção de Links Externos:** Uso rigoroso de `rel="noopener noreferrer"` em todos os links com `target="_blank"` para evitar ataques de *Reverse Tabnabbing*.
3. **Sanitização:** Uso de `textContent` ao invés de `innerHTML` em inputs de usuário para evitar XSS.
...
1. Clone este repositório:
   ```bash
   git clone [https://github.com/huankzera/wiki-suporte-isp.git](https://github.com/huankzera/wiki-suporte-isp.git)
